...
export function validateTaskForm(data: TaskFormData): ValidationResult {
  ...
  // 验证预计番茄数
  if (
    data.estimatedPomodoros < VALIDATION_RULES.estimatedPomodoros.min ||
...
export function validateField(
  ...
  switch (fieldName) {
    case 'title':
      ...
      if (
        typeof value === 'string' &&
        value.length > VALIDATION_RULES.title.maxLength
      ) {
    ...
    case 'notes':
      if (
        value &&
        typeof value === 'string' &&
        value.length > VALIDATION_RULES.notes.maxLength
      ) {
        return '备注不能超过500个字符';
      }
      return '';
