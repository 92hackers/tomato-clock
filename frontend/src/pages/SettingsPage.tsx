...
export const SettingsPage: React.FC = () => {
  ...
  // 时间设置处理（这里可以扩展为弹窗编辑）
  const handleTimeSettingClick = (setting: 'work' | 'shortBreak' | 'longBreak') => {
    // 临时实现：弹窗输入
    const currentValue = setting === 'work' 
    ...
    if (newValue && !isNaN(parseInt(newValue))) {
      ...
      if (minutes > 0 && minutes <= 120) { // 限制1-120分钟
      ...
      <SettingsSection title="通知" data-testid="notification-settings">
        ...
        >
          <ToggleSwitch
            ...
            onChange={(checked) => updateNotificationSettings({ timeEndAlert: checked })}
        ...
        >
          <ToggleSwitch
            ...
            onChange={(checked) => updateNotificationSettings({ breakReminder: checked })}
